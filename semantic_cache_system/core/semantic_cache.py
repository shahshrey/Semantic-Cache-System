...
class SemanticCache:
    ...
    def __init__(
        ...
        try:
            ...
            # Statistics tracking
            self.stats = {"hits": 0, "misses": 0, "adds": 0, "evictions": 0}
    ...
    def add_to_cache(self, query: str, response: str) -> bool:
        ...
        try:
            ...
            metadata = {"timestamp": time.time(), "query": query}
    ...
    def get_from_cache(self, query: str, k: int = 3) -> Tuple[Optional[str], float, Optional[float]]:
        ...
        try:
            ...
            response_results = self.vector_store.similarity_search(
                query=original_query, k=5, filter={"is_response": True}
            )
            ...
            if not response_results:
                ...
                if self.verbose:
                    print(
                        f"{COLORS['YELLOW']}Found query but no response for: {original_query[:50]}...{COLORS['RESET']}"
                    )
                return None, 0.0, None
            ...
            if self.verbose:
                print(
                    f"{COLORS['GREEN']}Cache hit for: {query[:50]}... (similarity: {similarity:.4f}){COLORS['RESET']}"
                )
